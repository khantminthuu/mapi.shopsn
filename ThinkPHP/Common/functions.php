...
function session($name, $value='') {
    ...
    $sessionObj = SessionGet::getInstance($name, $value, $prefix);
    if (is_null($name)) {
        // 清除
        $sessionObj->clear();
    } elseif ('' === $value) {
        // 判断或获取
        return 0 === strpos($name, '?') ? $sessionObj->has(substr($name, 1)) : $sessionObj->get();
    } elseif (is_null($value)) {
        // 删除
        return $sessionObj->delete();
    } else {
        // 设置
        return $sessionObj->set();
    }
}
...
function think_filter(&$value){
    ...
    if ($value != strip_tags($value)) { // 存在HTML标签
