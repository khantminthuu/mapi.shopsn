...
abstract class Driver {
     ...
     * @access public
     */
    public function connect($config='',$linkNum=0,$autoConnection=false) :\PDO
    {
     ...
     * @return mixed
     */
    public function query($str, bool $fetchSql=false) :array{
     ...
     * @return mixed
     */
    public function execute($str,$fetchSql=false) :bool
    {
        ...
        if(!empty($this->bind)){
            $that   =   $this;
            $this->queryStr =   strtr($this->queryStr,array_map(function($val) use($that){ if (!is_string($val)) {return $val;} return '\''.$that->escapeString($val).'\''; },$this->bind));
        }
        if($fetchSql){
            return false;
        }
     ...
     * @return string
     */
    protected function parseSet($data) :string{
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                for($i=0;$i<$count;$i++) {
                    ...
                    if(isset($val[$i][0]) && 'exp'===strtolower($val[$i][0])) {
     ...
     * @return string
     */
    protected function parseComment($comment) :string
    {
     ...
     */
    public function insert($data,$options=array(),$replace=false) :bool
    {
        $values  =  $fields    = array();
     ...
     * @return false | integer
     */
    public function insertAll($dataSet,$options=array(),$replace=false) :bool
    {
        $values  =  array();
        $this->model  =   $options['model'];
     ...
     * @return false | integer
     */
    public function update($data,$options) :bool{
        $this->model  =   $options['model'];
        $this->parseBind(!empty($options['bind'])?$options['bind']:array());
     ...
     * @return false | integer
     */
    public function delete($options=array()) :bool
    {
        $this->model  =   $options['model'];
        $this->parseBind(!empty($options['bind'])?$options['bind']:array());
        $table  =   $this->parseTable($options['table']);
     ...
     * @return string
     */
    public function parseSql($sql,$options=array()) :string
    {
        $sql   = str_replace(
     ...
     * @return void
     */
    protected function multiConnect($master=false) :\PDO{
        // 分布式数据库配置解析
