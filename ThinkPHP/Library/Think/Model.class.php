...
class Model {
     ...
     * @return mixed
     */
    public function select($options=array()) :array
    {
        $pk   =  $this->getPk();
     ...
     * @return array
     */
    public function parseFieldsMap($data,$type=1) :array{
        // 检查字段映射
        if(!empty($this->_map)) {
            foreach ($this->_map as $key=>$val){
                if($type==1) { // 读取
     ...
     * @return mixed
     */
     public function create($data='',$type='') :array
     {
        // 如果没有传值默认取POST数据
        if(empty($data)) {
            $data   =   I('post.');
        ...
        // 数据自动验证
        if(!$this->autoValidation($data,$type)) return [];
     ...
     * @return boolean
     */
    protected function autoValidation($data,$type) :bool{
     ...
     * @return boolean
     */
    public function check($value,$rule,$type='regex') :bool{
     ...
     * @return mixed
     */
    public function query($sql,$parse=false) :array
    {
     ...
     * @return false | integer
     */
    public function execute($sql,$parse=false) :bool{
        if(!is_bool($parse) && !is_array($parse)) {
            $parse = func_get_args();
            array_shift($parse);
        }
     ...
     * @return string
     */
    protected function parseSql($sql,$parse) :string
    {
        // 分析表达式
        if(true === $parse) {
     ...
     * @return Model
     */
    public function using($using) :self
    {
        $prefix =   $this->tablePrefix;
        if(is_array($using)) {
     ...
     * @return Model
     */
    public function join($join,$type='INNER') :self
    {
        $prefix =   $this->tablePrefix;
        if(is_array($join)) {
     ...
     * @return Model
     */
    public function union($union,$all=false) :self
    {
     ...
     * @return Model
     */
    public function cache($key=true,$expire=null,$type='') :self
    {
     ...
     * @return Model
     */
    public function scope($scope='',$args=NULL) :self
    {
        if('' === $scope) {
     ...
     * @return Model
     */
    public function where($where,$parse=null) :self
    {
     ...
     * @return Model
     */
    public function page($page,$listRows=null) :self
    {
     ...
     * @return Model
     */
    public function fetchSql($fetch=true) :self
    {
     ...
     * @return Model
     */
    public function bind($key,$value=false) :self
    {
        if(is_array($key)){
     ...
     * @return Model
     */
    public function setProperty($name,$value) :string
    {
