...
use Common\Model\OrderReturnGoodsModel;
...
class OrderReturnGoodsLogic extends AbstractGetDataLogic
{
    ...
    public function applyForAfterSale(){
        ...
        if (!$res) {
        	...
        	$rest = M('orderReviewProgress')->add($date);
        	if (!$rest) {
        		M()->rollback();
            	return array("status"=>0,"message"=>"申请失败","data"=>"");
        	}
    ...
    public function returnInfo(){
    	...
    	$goods['message'] = M('order_review_progress')->where(["return_id"=>$goods['id']])->order("create_time DESC")->getField("approval_content");
    	$speed = M('order_review_progress')->field("id,create_time,approval_content,approval_id")->where(["return_id"=>$goods['id']])->order("create_time DESC")->select();
