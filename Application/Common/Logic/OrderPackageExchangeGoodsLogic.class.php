...
use Common\Model\OrderPackageExchangeGoodsModel;
 ...
 *
 */
class OrderPackageExchangeGoodsLogic extends AbstractGetDataLogic
{
    /**
     * 构造方法
     * @param array $data
    ...
    public function __construct(array $data = [], $split = '')
    {
        $this->data = $data;
        $this->splitKey = $split;
        $this->modelObj = new OrderPackageExchangeGoodsModel();
    }
    /**
     * 返回验证数据
     */
    ...
    public function applyForAfterSale(){
        ...
        if (!$res) {
        	...
        	$rest = M('order_package_exchange_progress')->add($date);
        	if (!$rest) {
        		M()->rollback();
            	return array("status"=>0,"message"=>"申请失败","data"=>"");
        	}
    ...
    public function returnInfo(){
    	...
    	$goods = $this->goodsModel->getTitleByOneData($return);
    	$goods['message'] = M('order_package_exchange_progress')->where(["exchange_id"=>$goods['id']])->order("create_time DESC")->getField("approval_content");
    	$speed = M('order_package_exchange_progress')->field("id,create_time,approval_content,approval_id")->where(["exchange_id"=>$goods['id']])->order("create_time DESC")->select();
