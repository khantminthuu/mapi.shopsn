...
class AopClient {
	...
	protected function curl($url, $postFields = null) {
		...
		if ($postMultipart) {
			...
			$headers = array (
					'content-type: multipart/form-data;charset=' . $this->postCharset . ';boundary=' . $this->getMillisecond () 
			);
		} else {
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		file_put_contents ( './Log/Alipay.txt', print_r ( $logData, true ), FILE_APPEND );
	}
	...
	public function pageExecute($request, $httpmethod = "POST") {
		...
		if (method_exists ( $request, "getNeedEncrypt" ) && $request->getNeedEncrypt ()) {
			...
			// 执行加密
			$enCryptContent = base64_encode ( $apiParams ['biz_content'] . $this->encryptKey );
	...
	private function encryptJSONSignSource($request, $responseContent) {
		...
		$bizContent = base64_decode ( $parsetItem->encryptContent . $this->encryptKey );
